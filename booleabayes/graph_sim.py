...
### Copied from random_walk_raw, but using state, not state_idx
def random_walk_state(start_bool, rules, regulators_dict, nodes, steps, on_nodes=[], off_nodes=[]):
    walk = pd.DataFrame(index=nodes, columns=list(range(steps)))
    n = len(nodes)
    node_indices = dict(zip(nodes,range(len(nodes))))
    ...
    next_step = [i for i in start_bool]
    walk[0] = next_step
    
    for col in walk.columns[1:]:
        r = np.random.rand()
        for node_i, node in enumerate(nodes):
            if node in on_nodes+off_nodes: continue
            flip = get_update_prob(rules, regulators_dict, node, node_i, node_indices, next_step)
            #r = r - flip**2/(1.*nu) # **2 seems like it makes steps less likely?
            r = r - flip/(1.*nu)
            if r <= 0:
                next_step[node_i] = not next_step[node_i]
                flipped_nodes.append(node)
                break
        if r > 0: flipped_nodes.append(None)
        walk[col] = next_step
    return walk, flipped_nodes
...
def get_update_prob(rules, regulators_dict, node, node_i, node_indices, state_bool):
    # Get rule and regulators for this node
    rule = rules[node]
    regulators = regulators_dict[node]

    # Get the state and "leaf-index" of regulators
    regulator_indices = [node_indices[i] for i in regulators]
    regulator_state = [state_bool[i] for i in regulator_indices]
    rule_leaf = state_bool2idx(regulator_state)

    # Get the flip probability (rule[leaf] is prob from 0->1, if already ON, must invert)
    flip = rule[rule_leaf]
    if state_bool[node_i]: return 1-flip
    else: return flip
